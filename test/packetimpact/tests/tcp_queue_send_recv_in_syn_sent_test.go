...
func TestQueueRecvInSynSentHandshake(t *testing.T) {
	...
	if _, _, err := dut.RecvWithErrno(context.Background(), t, socket, int32(len(sampleData)), 0); err != unix.Errno(unix.EWOULDBLOCK) {
		t.Fatalf("expected error %s, got %s", unix.Errno(unix.EWOULDBLOCK), err)
	}
...
func TestQueueSendInSynSentRST(t *testing.T) {
	...
	go func() {
		...
		// process until the connection is established.
		n, err := dut.SendWithErrno(ctx, t, socket, sampleData, 0)
		if err != unix.Errno(unix.ECONNREFUSED) {
			t.Errorf("expected error %s, got %s", unix.Errno(unix.ECONNREFUSED), err)
		}
		if n != -1 {
