...
func TestTCPNagleMSS(t *testing.T) {
	...
	expectedPayload := testbench.Payload{Bytes: expectedData}
	if _, err := conn.ExpectData(t, &testbench.TCP{Flags: testbench.Uint8(header.TCPFlagPsh | header.TCPFlagAck)}, &expectedPayload, time.Second); err != nil {
