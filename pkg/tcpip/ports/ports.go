...
type PortManager struct {
	...
	// ephemeralMu protects firstEphemeral and numEphemeral. It should be
	// locked after mu.
...
func NewPortManager() *PortManager {
	return &PortManager{
		...
		// Match Linux's default ephemeral range.
		firstEphemeral: 32769,
		numEphemeral:   28232,
	}
}
...
func (s *PortManager) PickEphemeralPort(testPort func(p uint16) (bool, tcpip.Error)) (port uint16, err tcpip.Error) {
	...
	return s.pickEphemeralPort(offset, firstEphemeral, numEphemeral, testPort)
}

// portHint atomically reads and returns the s.hint value.
func (s *PortManager) portHint() uint16 {
	return uint16(atomic.LoadUint32(&s.hint) % math.MaxUint16)
}
...
func (s *PortManager) PickEphemeralPortStable(offset uint32, testPort func(p uint16) (bool, tcpip.Error)) (port uint16, err tcpip.Error) {
	...
	// TODO: Take a uint16 offset plz
	p, err := s.pickEphemeralPort(s.portHint()+uint16(offset), firstEphemeral, numEphemeral, testPort)
	if err == nil {
		s.incPortHint()
	}
	return p, err
...
func (s *PortManager) pickEphemeralPort(offset, first, count uint16, testPort func(p uint16) (bool, tcpip.Error)) (port uint16, err tcpip.Error) {
